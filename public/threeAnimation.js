...
objLoader.load(
  ...
  ({ children }) => {
    ...
    addCube(children[0], prevSlide, screenBorderRight - 2.4, screenBottom + 1)
...
const addCube = (object, callbackFn, x, y) => {
  ...
  arrowBox2.push(boundingBox)
  scene.add(boundingBox)
}

const animate = () => {
  arrowBox2.forEach((element, i) => {
    arrowBoxRotation2[i] = lerp(arrowBoxRotation2[i], 0, .05)
    element.rotation.set(THREE.Math.degToRad(arrowBoxRotation2[i]), 0, 0)
  })
...
export const handleThreeAnimation = (i) => {
  i == 0 ? arrowBoxRotation2[0] = 360 : arrowBoxRotation2[1] = -360;
}
